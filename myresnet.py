...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, group=1, base_width=64,
                 dilation=1, norm_layer=None):
        super(BasicBlock, self).__init__()
        if not norm_layer:
            norm_layer = nn.BatchNorm2d
        if group != 1 or base_width != 64:
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
                 base_width=64, dilation=1, normal_layer=None):
        super(Bottleneck, self).__init__()
        if not normal_layer:
            normal_layer = nn.BatchNorm2d
        width = int(planes * (base_width / 64.)) * groups
        self.conv1 = conv1x1(inplanes, width)
        self.bn1 = normal_layer(width)
        self.conv2 = conv3x3(width, width, stride, groups, dilation)
        self.bn2 = normal_layer(width)
        self.conv3 = conv1x1(width, planes * self.expansion)
        self.bn3 = normal_layer(planes * self.expansion)
        self.relu = nn.ReLU(inplace=True)
        self.downsample = downsample
        self.stride = stride
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=10, zero_init_residual=False,
        ...
        if len(replace_stride_with_dilation) != 3:
            raise ValueError("replace_stride_with_dilation should be 0 or a "
